...
/*   Updated: 2023/12/13 19:39:33 by rikeda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
									...
									const char *identifer)
{
			...
			|| ft_is_equal_str(identifer, ID_QUADRIC)))
	{
		...
		return (check_size_of_parameters(rt_object, TRIANGLE_MAX_SIZE));
	else if (ft_is_equal_str(identifer, ID_QUADRIC))
		return (check_size_of_parameters(rt_object, QUADRIC_MAX_SIZE));
	else
	{
...
static int	_check_rt_object(const t_vla *rt_object)
{
	...
	if (ft_is_equal_str(identifer, ID_AMBIENT))
		return (check_size_of_parameters(rt_object, AMBIENT_MAX_SIZE));
	else if (ft_is_equal_str(identifer, ID_CAMERA))
		return (check_size_of_parameters(rt_object, CAMERA_MAX_SIZE));
	else if (ft_is_equal_str(identifer, ID_SPOT))
		return (check_size_of_parameters(rt_object, SPOT_MAX_SIZE));
	else if (ft_is_equal_str(identifer, ID_CYLINDER))
		return (check_size_of_parameters(rt_object, CYLINDER_MAX_SIZE));
	else if (ft_is_equal_str(identifer, ID_PLANE))
		return (check_size_of_parameters(rt_object, PLANE_MAX_SIZE));
	else if (ft_is_equal_str(identifer, ID_SPHERE))
		return (check_size_of_parameters(rt_object, SPHERE_MAX_SIZE));
	else
