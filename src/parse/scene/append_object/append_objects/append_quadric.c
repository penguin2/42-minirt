...
/*   Updated: 2023/11/14 16:06:11 by rikeda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	_append_quadric_object(t_quadric quadric, t_vla *objects)
{
	...
	new_quadric = quadric_new(quadric.center,
			quadric.coeff_quadratic,
			quadric.coeff_z,
			quadric.coeff_c);
...
static int	_try_json_node_to_quadric(const t_json_node *node,
										t_quadric *quadric)
{
	...
	if (json_node_to_double(
			k_a_node, &quadric->center.x, NO_LIMIT, NO_LIMIT) == ERROR
		|| json_node_to_double(
			k_b_node, &quadric->center.y, NO_LIMIT, NO_LIMIT) == ERROR
		|| json_node_to_double(
			k_c_node, &quadric->center.z, NO_LIMIT, NO_LIMIT) == ERROR
		|| json_node_to_double(
			k_d_node, &quadric->coeff_c, NO_LIMIT, NO_LIMIT) == ERROR
		|| json_node_to_double(
			k_e_node, &quadric->coeff_z, NO_LIMIT, NO_LIMIT) == ERROR)
		return (ERROR);
	else
		return (SUCCESS);
}
...
int	append_quadric(const t_json_node *node, t_vla *objects)
{
	...
	if (list_to_vec3(get_list(node, COORDINATES, 3),
		...
		|| _try_json_node_to_quadric(node, &quadric) == ERROR)
		return (ERROR);
