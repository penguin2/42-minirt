...
/*   Updated: 2023/11/13 18:03:52 by taekklee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	list_to_color(const t_vla *list, t_color *color)
{
	if (list == NULL || list->size != 3 || !is_only_value_node(list))
	{
		...
		return (ERROR);
	if (is_between_min_to_max(color->red, 0.0, MAX_COLOR_8BIT)
		&& is_between_min_to_max(color->green, 0.0, MAX_COLOR_8BIT)
		&& is_between_min_to_max(color->blue, 0.0, MAX_COLOR_8BIT))
	{
		color->red /= MAX_COLOR_8BIT;
		color->green /= MAX_COLOR_8BIT;
		color->blue /= MAX_COLOR_8BIT;
		return (SUCCESS);
	}
	else
		return (ERROR);
